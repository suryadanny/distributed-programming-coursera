...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
    	...
    	JavaPairRDD<Integer,Double> updatedRanksList =sites.join(ranks).flatMapToPair(pair -> {
    	    final List<Tuple2<Integer,Double>> contribs = new LinkedList<Tuple2<Integer,Double>>();
        	
    	    Website site = pair._2._1;
    	    Double rank = pair._2._2;
    		Iterator<Integer> edges = site.edgeIterator();
    	    while(edges.hasNext())	
    	     	contribs.add(new Tuple2(edges.next(),rank/site.getNEdges()));
    	...
    	return updatedRanksList.reduceByKey((Double v1, Double v2) -> ( v1+v2)).mapValues(v -> (0.15+0.85*v));
